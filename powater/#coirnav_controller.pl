...
sub EVENT_SIGNAL {
  if ($signal == 1) { #signal on death of #Guardian_of_Coirnav
    ...
    if(plugin::ReadQGlobal("coirnav_wave") == 4) { # must be on wave 4
    ...
    if (plugin::ReadQGlobal("coirnav_wave") == 3) { #make sure we're on 3rd wave
    ...
    quest::setglobal("coirnav_done", 3, 7, "H8"); # You can't attempt this again for 5 days
...
sub EVENT_TIMER {
  if($timer == 1) { # This gets called when you fail because of timeout
    ...
    quest::depop(216109);
    quest::depop(216110);
    quest::setglobal("coirnav_done", 3, 7, "M30"); # You can't attempt this again for 2 hours
...
sub SPAWN_WAVE1 {
  ...
  # AD Custom
  quest::settimer(1,1200); # 20 Minute timer on entire event
  quest::settimer(2,240); # 4 Minute timer for second wave
  quest::settimer(3,360); # 6 Minute timer for third wave
  quest::settimer(4,900); # 15 Minute timer for the emote
  quest::settimer(5,1020); # 17 Minute timer for the emote
  quest::settimer(6,1140); # 19 Minute timer for the emote
