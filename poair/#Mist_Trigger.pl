...
sub EVENT_SIGNAL 
{
	if ($signal == 1 && defined(plugin::ReadQGlobal("mist_event_start")) && !defined(plugin::ReadQGlobal("mist_done"))) 
	{
     		$counter+=1;
     	...
     	elsif ($signal == 2 && defined(plugin::ReadQGlobal("mist_event_start")) && !defined(plugin::ReadQGlobal("mist_done"))) 
     	{
        	quest::spawn2(215071,0,0,-1571.6,-570.0,343.6,190.9);
		quest::depop_withtimer(215023);
     	}
     	elsif ($signal == 3  && !defined(plugin::ReadQGlobal("mist_done"))) 
     	{
        	quest::setglobal("mist_done",1,3,"F");
     	}
	elsif ($signal == 4 && !defined(plugin::ReadQGlobal("mist_event_start")) && !defined(plugin::ReadQGlobal("mist_done"))) 
	{
