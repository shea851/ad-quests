...
sub EVENT_SIGNAL 
{
	if ($signal == 1 && defined(plugin::ReadQGlobal("smoke_event_start")) && !defined(plugin::ReadQGlobal("smoke_done")))
	{
		$counter+=1;
     	...
     	elsif ($signal == 2 && defined(plugin::ReadQGlobal("smoke_event_start")) && !defined(plugin::ReadQGlobal("smoke_done")))
     	{
     		quest::spawn2(215435,0,0,1395.9,-681.3,15.1,255.1);
        	quest::depop_withtimer(215055);
     	}
     	elsif ($signal == 3  && !defined(plugin::ReadQGlobal("smoke_done")))
     	{
        	quest::setglobal("smoke_done",1,3,"F");
     	}
	elsif ($signal == 4 && !defined(plugin::ReadQGlobal("smoke_event_start")) && !defined(plugin::ReadQGlobal("smoke_done")))
	{
