...
sub EVENT_SIGNAL {
 ...
 if($signal == 2) {
    $counter += 1;		
if($signal == 2 && $counter == 4 && !defined $qglobals{dust_ring} && defined plugin::ReadQGlobal("dust_ring_run")) {
      	quest::spawn_condition($zonesn,1,1); 
	}
	$check_boss = $entity_list->GetMobByNpcTypeID(218064);
	if($signal == 2 && !$check_boss && !defined $qglobals{dust_ring} && defined plugin::ReadQGlobal("dust_ring_run")) {
 ...
 if($signal == 3 && defined plugin::ReadQGlobal("dust_ring_run")) {
    $countertwo += 1;
	if($signal == 3 && $countertwo == 3 && !defined $qglobals{dust_ring} && defined plugin::ReadQGlobal("dust_ring_run")) {
	$countertwo=0;
	quest::spawn2(218096,0,0,0,-520,30.5,0);
	}
 }

 if($signal == 4 && defined plugin::ReadQGlobal("dust_ring_run")) {
 	quest::spawn_condition($zonesn,1,0);
   	quest::delglobal("dust_ring");
   	quest::setglobal("dust_ring",1,3,"H8");
        quest::setglobal("dust_ring_final",1,3,"H8");
