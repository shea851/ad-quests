...
sub EVENT_TIMER {
    ...
    $check_boss = $entity_list->GetMobByNpcTypeID(218019); #A_Tainted_Rock_Beast 
        ...
        if (!$check_boss && !defined $qglobals{vine_ring} && !defined plugin::ReadQGlobal("vine_ring_run")) { #When no more A_Tainted_Rock_Beast are up, spawn the A_Bloodthirsty_Vegerog 
...
sub EVENT_SIGNAL {
	if($signal == 1) { #signal 1 is from the A_Bloodthirsty_Vegerog 
		$counter += 1;
		# counter used to be 10
		if($counter == 6 && !defined $qglobals{vine_ring} && defined plugin::ReadQGlobal("vine_ring_run")) {
			quest::spawn2(218058,0,0,485,-835,30,0);
			$counter=0;
		}
	}

	if($signal == 3 && !defined $qglobals{vine_ring} && defined plugin::ReadQGlobal("vine_ring_run")) { #signal 3 is from Derugoak_Bloodwalker 
		quest::delglobal("vine_ring");
		quest::delglobal("vine_ring_run");
		quest::setglobal("vine_ring",1,3,"H8"); #Event is over
                quest::setglobal("vine_ring_final",1,3,"H8");
