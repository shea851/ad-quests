...
sub EVENT_SAY {
  #$faction = $client->GetCharacterFactionLevel(342); # This never worked out-of-box
  ...
  if ($text=~/decrepit sheath/i && $class eq "Shadowknight") {
...
sub EVENT_ITEM {
  #$faction = $client->GetCharacterFactionLevel(342); # This never worked out-of-box
  if ($class eq "Shadowknight" && plugin::check_handin(\%itemcount, 14372 => 1, 14371 => 1, 16507 => 1)) {
